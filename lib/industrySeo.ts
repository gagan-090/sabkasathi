...
export interface IndustryServiceInfo {
  ...
  slug: string;
  group: string;
  /** "an app", "a billing system" — reads naturally after "we build". */
  article: string;
...
export function getIndustryContentBySlug(slug: string): IndustryPageData | null {
  ...
  const whyThisIndustry =
    ...
    `the ${service.name.toLowerCase()} is shaped to that. ${service.outcome}`;
  ...
  const pains = industry.pains.map((pain, i) => ({
    pain,
    fix: `${service.modules[i % service.modules.length]} — built into the ${service.name.toLowerCase()} rather than bolted on afterwards.`,
  }));
  ...
  const faqs = [
    {
      q: `How much does ${title.toLowerCase()} cost in India?`,
      a:
    ...
    },
    {
      ...
      a:
        `A typical ${industry.name.toLowerCase()} ${service.name.toLowerCase()} project takes ${service.timeline} from ` +
    ...
    },
    {
      ...
      a:
        `Because ${industry.context}. ${capitalise(industry.pains[0].toLowerCase())} is the direct consequence, ` +
        `and it is not something a generic product solves — it needs ${lowerFirst(industry.systems[0])}. ` +
        `${service.outcome}`,
    },
    {
